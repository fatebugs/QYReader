...
import top.cronos.myreader.entity.ad.AdConfig;
import top.cronos.myreader.model.user.User;
import top.cronos.myreader.model.user.UserService;
import top.cronos.myreader.util.SharedPreAdUtils;
...
public class AdUtils {
    ...
    public static void resetPangleId() {
        ...
        if (shared.exists() && shared.isDirectory()) {
            File[] files = shared.listFiles((dir, name) -> !name.startsWith("QYReader"));
            for (File file :files) {
                file.delete();
            }
        }
